...
#ifdef CONFIG_FIH_DRG
// Workaround for DRG external sense issue
...
#ifdef CONFIG_FIH_DRG
	if(chip->rsense_rw == 1)
		device_create_file(&pdev->dev, &dev_attr_rsense_sel);
#endif
...
#ifdef CONFIG_FIH_DRG
	if(chip->rsense_rw == 1)
		device_remove_file(&pdev->dev, &dev_attr_rsense_sel);
#endif
